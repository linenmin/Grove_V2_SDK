...
class MultiScaleResNet(BaseLayers):
    ...
    def CompressedAdditiveSkip(self, decoder_inputs, encoder_inputs, filters, name):
        hidden_filters = max(filters // 2, 8)
        projected = self.Conv(
        ...
        pad_h = decoder_shape[1] - projected_shape[1]
        pad_w = decoder_shape[2] - projected_shape[2]
        if pad_h < 0 or pad_w < 0:
            raise ValueError(
                f"Skip shape mismatch is not pad-only: decoder={decoder_shape}, "
                f"projected={projected_shape}"
            )
        if pad_h > 0 or pad_w > 0:
            projected = tf.pad(
                projected,
        ...
        return tf.add(decoder_inputs, projected, name=name + "_add")
    ...
    @CountAndScope
    def ECABlock(self, inputs=None, kernel_size=3, name="eca"):
        channels = inputs.get_shape().as_list()[-1]
        if channels is None:
            raise ValueError("ECA block requires static channel dimension")

        pooled = tf.reduce_mean(inputs, axis=[1, 2], keepdims=True, name=name + "_mean")
        pooled_1d = tf.reshape(
            pooled,
            [-1, 1, channels, 1],
            name=name + "_reshape_to_1d",
        )
        ...
        attn = tf.reshape(
            attn,
            [-1, 1, 1, channels],
            name=name + "_reshape_back",
        )
        attn = tf.math.sigmoid(attn, name=name + "_sigmoid")
        return tf.multiply(inputs, attn, name=name + "_scale")
    ...
    def ResNetBlock(self, inputs):
        ...
        net = self.ECABlock(inputs=net, kernel_size=3, name="eca_bottleneck")
        ...
        for count in range(self.NumSubBlocks):
            ...
            if count == 0:
                net = self.CompressedAdditiveSkip(
                    decoder_inputs=net,
