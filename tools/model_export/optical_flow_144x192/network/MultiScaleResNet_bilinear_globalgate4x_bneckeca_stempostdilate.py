...
class MultiScaleResNet(BaseLayers):
    ...
    @CountAndScope
    def DilatedConvBNReLUBlock(self, inputs=None, filters=None, kernel_size=None, rate=1, name=None):
        if kernel_size is None:
            kernel_size = self.kernel_size
    ...
    @CountAndScope
    def StemPostDilatedBlock(self, inputs=None, filters=None, post_rate=2, name="stem_postdilated"):
        net = self.ConvBNReLUBlock(
            inputs=inputs,
            filters=filters,
            kernel_size=(3, 3),
        ...
        net = self.DilatedConvBNReLUBlock(
            inputs=net,
            filters=filters,
            kernel_size=(3, 3),
            rate=post_rate,
    ...
    def ResNetBlock(self, inputs):
        ...
        net = self.StemPostDilatedBlock(
            inputs=inputs,
            filters=num_filters,
            post_rate=3,
            name="E0_stempostdilate",
        )
