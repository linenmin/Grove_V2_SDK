...
- 已恢复 `EPII_CM55M_APP_S/makefile`：`APP_TYPE = optical_sd`
...
建议：对"SD-only 版本"单独保留最小依赖（FatFS + TFLM + SPI/串口必要模块），不编译不使用的 `event_handler` / `cisdp_sensor` 流程。
...
- 若只想优化"非模型部分"，先不动它；若追求总 RAM，再做逐步压测（每次减 32KB/64KB，验证 `AllocateTensors` 成败）。

### D. 栈与缓冲复用（中收益）

- 目前双帧缓存约 `RAW_FRAME_BYTES * 2 = 259200 bytes`（约 253KB），这是数据面必须成本。
- 可继续保持双缓冲复用策略，避免新增中间拷贝。
...
2) `optical_sd/tflm_yolov8_od.mk` 当前本就配置：`CIS_SUPPORT_INAPP_MODEL = cis_ov5647`
