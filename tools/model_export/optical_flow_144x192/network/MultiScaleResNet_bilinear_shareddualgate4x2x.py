...
class MultiScaleResNet(BaseLayers):
    ...
    @CountAndScope
    def GlobalContextVector(self, context_inputs=None, name="global_context"):
    ...
    @CountAndScope
    def ApplyProjectedGate(self, context_vector=None, target_inputs=None, target_filters=None, name="global_gate"):
        gate = self.Conv(
    ...
    def ResNetBlock(self, inputs):
        ...
        shared_context = self.GlobalContextVector(
            context_inputs=net,
            name="shared_dual_gate",
        )
        ...
        for count in range(self.NumSubBlocks):
            ...
            if count == 1:
                net = self.ApplyProjectedGate(
                    context_vector=shared_context,
                    target_inputs=net,
                    target_filters=num_filters,
                    name="shared_dual_gate_4x",
                )
