...
**验证结果**：
- 彩色光流和灰度光流都看不出棋盘格形状
- 又回到了条纹输出（与之前 plan-007 类似的问题）

**分辨率分析**：

| 阶段 | 分辨率 | 说明 |
|------|--------|------|
| 摄像头原始 | 640×480 | MIPI LINE/FRAME LENGTH |
| 模型输入 | 200×150×6 | resize（Helium）|
| 模型输出 | 208×160×2 | 光流 dx, dy |
| 光流可视化 | 208×160 | 直接使用模型输出 |

**数据处理流程**：
```
摄像头 (640×480 planar BGR)
    ↓ Helium resize (无裁剪)
模型输入 (200×150 RGB × 2帧交错 = 6通道)
    ↓ NPU 推理
模型输出 (208×160 × 2通道 dx,dy)
    ↓ HSV 映射
光流可视化 (208×160 彩色/灰度)
```
...
**问题分析**：
光流输出看不出棋盘格形状，可能原因：
1. **光流模型本身**：模型可能没有正确学习运动场
2. **输入预处理**：resize 或 NHWC 布局转换问题
3. **输出读取**：planar vs interleaved 读取方式

**下一步调查方向**：
1. 检查模型输出直方图 - 是否为常量场或窄分布
2. 验证 NHWC 布局转换是否正确
3. 对比离线 Python 脚本输出
