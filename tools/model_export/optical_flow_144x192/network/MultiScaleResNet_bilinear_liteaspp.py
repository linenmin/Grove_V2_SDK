...
class MultiScaleResNet(BaseLayers):
    ...
    @CountAndScope
    def LiteASPPBlock(self, inputs=None, filters=None):
        branch_filters = max(filters // 4, self.InitNeurons)

        branch_1x1 = self.ConvBNReLUBlock(
            inputs=inputs,
            filters=branch_filters,
            kernel_size=(1, 1),
            strides=(1, 1),
        )
        branch_r2 = self.DilatedConvBNReLUBlock(
            inputs=inputs,
            filters=branch_filters,
            kernel_size=(3, 3),
            rate=2,
            name="lite_aspp_rate2",
        )
        branch_r4 = self.DilatedConvBNReLUBlock(
            inputs=inputs,
            filters=branch_filters,
            kernel_size=(3, 3),
            rate=4,
            name="lite_aspp_rate4",
        )

        fused = tf.add(branch_1x1, branch_r2, name="lite_aspp_add_1")
        fused = tf.add(fused, branch_r4, name="lite_aspp_add_2")
        fused = self.Conv(
            inputs=fused,
            filters=filters,
            kernel_size=(1, 1),
        ...
        return tf.add(inputs, fused, name="lite_aspp_out")
    ...
    def ResNetBlock(self, inputs):
        ...
        net = self.LiteASPPBlock(inputs=net, filters=num_filters)
